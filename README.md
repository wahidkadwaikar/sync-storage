...
// In production, your JWT should include: { sub: \"my-user-id\" }
...
If running local/dev with `AUTH_MODE=none`, passing `userId` in the SDK sets `x-user-id` automatically.

## API example

For local/dev with `AUTH_MODE=none`, send `x-user-id`.
...
- Production default: `AUTH_MODE=jwt`
  ...
  - Uses JWT `tenant_id` when present, else `DEFAULT_TENANT_ID`
- Local/dev mode: `AUTH_MODE=none`
  - Requires `x-user-id`

`x-namespace` is required unless `DEFAULT_NAMESPACE` is configured.
